...
module.exports = {
    getLogin: async function(req, res) {
        ...
        let redirect = `${this.OAuth2.Google.redirect}&state=${state.id}`;
    ...
    postLogin: async function(req, res) {
        ...
        if (!userInfo || !userInfo.id) {
            ...
            if (result.error) {
                let refresh = await this.OAuth2.Google.exchange(user.OAuth2Refresh, true);

                if (refresh.error) {
                    this.logger.onError(`Google refused to exchange refresh token, description: ${refresh.error_description}`);
                    return void res.clearCookie(cookieName).status(500).send("Google Error (1)");
                }

                result = await this.OAuth2.Google.fetchUserInfo(refresh.access_token);

                if (result.error) {
                    this.logger.onError(`Google refused to exchange access token, description: ${refresh.error_description}`);
    ...
    callback: async function(req, res) {
        ...
        if (req.query.code) {
            let result = await this.OAuth2.Google.exchange(req.query.code, false);

            if (result.error) {
                this.logger.onError(`Google refused callback code: ${req.query.code}`);
                return void res.redirect("/500");
            }

            let userInfo = await this.OAuth2.Google.fetchUserInfo(result.access_token);

            if (userInfo.error) {
                this.logger.onError(`Google refused access code at callback ${req.query.code} ${result.error_description}`);
                return void res.redirect("/500");
            }
